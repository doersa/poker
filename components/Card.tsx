...
const Card: React.FC<CardProps> = ({ card, hidden, className = '', style, flipOnMount, flipDelay = 0 }) => {
  ...
  const CardBack = () => (
    <div 
      className="w-full h-full bg-blue-900 rounded-md border-2 border-gray-300 shadow-md relative overflow-hidden"
    >
      {/* Repeating Spade Pattern - Rotated for style */}
      <div 
      ...
      >
         <div 
            ...
            style={{
                backgroundImage: `url("data:image/svg+xml,%3Csvg width='20' height='20' viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg'%3E%3Ctext x='50%25' y='55%25' font-size='12' fill='%23ffffff' font-family='Arial, sans-serif' text-anchor='middle' dominant-baseline='central'%3E♠%3C/text%3E%3C/svg%3E")`,
      ...
      {/* Center Emblem */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-6 h-6 sm:w-8 sm:h-8 rounded-full border border-blue-300/20 flex items-center justify-center bg-blue-950/60 backdrop-blur-[2px] shadow-lg">
        <span className="text-blue-100/70 text-xs sm:text-sm shadow-black drop-shadow-md">♠</span>
      </div>
    </div>
  );
